...
import { ERAS, MATHEMATICIANS, byId } from './data.js'
import {
...
const ALL_EDGES = influenceEdges(MATHEMATICIANS)
const DEGREE = Object.fromEntries(
  MATHEMATICIANS.map(p => [p.id, p.influences.length + p.influenced.length]),
)
const LABEL_BELOW = Object.fromEntries(MATHEMATICIANS.map((p, i) => [p.id, i % 2 === 1]))
const clampZoom = z => Math.max(1, Math.min(8, z))

// Chalk on slate: INK is chalk, PAPER is the board.
const INK = '#eae7dc'
const INK_LAND = '#c9c6ba'
const PAPER = '#1c2823'
const FADE = 'rgba(234,231,220,.38)'

// Chalkboard fill tiles, built once: bare-slate ocean, chalk-dusted land.
...
function buildPatterns(ctx) {
  ...
  oceanPattern = tile(4, 4, t => {
    t.fillStyle = '#1a2621'
    t.fillRect(0, 0, 4, 4)
    t.strokeStyle = 'rgba(234,231,220,.07)'
    t.lineWidth = 0.45
    t.beginPath()
    t.moveTo(0, 2)
    t.lineTo(4, 2)
  ...
  landPattern = tile(5, 5, t => {
    t.fillStyle = '#233028'
    t.fillRect(0, 0, 5, 5)
    t.fillStyle = 'rgba(234,231,220,.30)'
    t.beginPath()
    t.arc(1.2, 1.4, 0.5, 0, 7)
    t.fill()
    t.fillStyle = 'rgba(234,231,220,.18)'
    t.beginPath()
...
export default function Globe({ selectedId, onSelect, itinerary }) {
  ...
  function draw() {
    ...
    ctx.strokeStyle = 'rgba(234,231,220,.20)'
    ctx.lineWidth = 0.35
    ctx.stroke()
    ctx.setLineDash([])
    ...
    if (cons) {
      ...
      ctx.strokeStyle = 'rgba(234,231,220,.30)'
      ctx.lineWidth = 0.55
      ctx.stroke()
    }
    ...
    if (trip) {
      ...
      ctx.setLineDash([7, 5])
      ctx.strokeStyle = 'rgba(232,212,139,.65)'
      ctx.lineWidth = 1.4
      ctx.stroke()
      ctx.setLineDash([])
      trip.ids.forEach((id, i) => {
        ...
        ctx.arc(px, py, 6, 0, 7)
        if (i <= trip.upTo) {
          ctx.fillStyle = 'rgba(232,212,139,.85)'
          ctx.fill()
        } else {
          ctx.strokeStyle = 'rgba(232,212,139,.7)'
          ctx.lineWidth = 1.2
          ctx.stroke()
        }
      })
    ...
    if (selPhil) {
      ...
      const arc = (otherId, dashed) => {
        ...
        ctx.strokeStyle = dashed
          ? `rgba(234,231,220,${cons ? 0.75 : 0.55})`
          : `rgba(234,231,220,${cons ? 0.9 : 0.72})`
    ...
    for (const p of MATHEMATICIANS) {
      ...
      if (connected.has(p.id)) {
        ...
        ctx.strokeStyle = 'rgba(224,169,164,.85)'
        ctx.lineWidth = 1
        ctx.stroke()
        ctx.setLineDash([])
      }
      if (medallions) {
        ...
        ctx.lineWidth = 0.6
        ctx.stroke()
        if (zoom >= 4) label(ctx, p.name, px, py + 24, '9px Chalkduster, "Chalkboard SE", cursive')
        ctx.globalAlpha = 1
      } else if (lens) {
        ctx.beginPath()
        ...
        if (lit) {
          ...
          ctx.lineWidth = 1.4
          ctx.stroke()
          label(ctx, p.name, px, py + (LABEL_BELOW[p.id] ? 20 : -11), '10px Chalkduster, "Chalkboard SE", cursive')
        } else {
        ...
        ctx.beginPath()
        ctx.arc(px, py, r, 0, 7)
        ctx.fillStyle = 'rgba(242,239,230,.92)'
        ctx.fill()
        if (tier === 3) {
          ctx.strokeStyle = 'rgba(242,239,230,.5)'
          ctx.lineWidth = 0.5
          ctx.beginPath()
        ...
        ctx.beginPath()
        ctx.arc(px, py, 4, 0, 7)
        ctx.fillStyle = 'rgba(242,239,230,.88)'
        ctx.fill()
        ctx.strokeStyle = INK
        ctx.lineWidth = 1.2
        ctx.stroke()
    ...
    if (hov && hov !== selectedRef.current) {
      ...
      if (h) label(ctx, byId[hov].name, h.x, h.y - (medallions ? 18 : 11), '10.5px Chalkduster, "Chalkboard SE", cursive')
    }
    ...
    if (selId) {
      ...
      if (h) {
        ...
        ctx.lineWidth = 0.8
        ctx.stroke()
        label(ctx, `${p.name} · ${p.place.name}`, h.x, h.y + 47, '11.5px Chalkduster, "Chalkboard SE", cursive')
      }
    }
  }
  ...
  return (
    <div className="globe-wrap">
      ...
      <div className="scrub">
        ...
        {lensOn && (
          <img
            className="era-emblem"
            src={`emblems/${era.id}.jpg`}
            alt=""
      ...
      <div className="globehint">
        {selectedId
          ...
          : 'drag to turn · scroll to zoom · scrub to light an era · click a mathematician to open their entry'}
      </div>
    </div>
  )
}
